...
def test():
    ...
    myobj = {"userID": 78, "movieID": 903, "rating": 4.0, "genre-Action": 0, "genre-Adventure": 0, "genre-Animation": 0,"genre-Children": 0, "genre-Comedy": 0, "genre-Crime": 0, "genre-Documentary": 0, "genre-Drama": 1,"genre-Fantasy": 0, "genre-Film-Noir": 0, "genre-Horror": 0, "genre-IMAX": 0, "genre-Musical": 0,"genre-Mystery": 1, "genre-Romance": 1, "genre-Sci-Fi": 0, "genre-Short": 0, "genre-Thriller": 1, "genre-War":0, "genre-Western": 0}
    reqlist = []
    x = requests.post(url+'rating', data = myobj)
    reqlist.append(x)
    y = requests.get(url+'ratings')
    reqlist.append(y)
    y1 = requests.get(url + 'rating')
    reqlist.append(y1)
    y2 = requests.get(url + 'avg-genre-ratings/all-users')
    reqlist.append(y2)
    y3 = requests.get(url + 'avg-genre-ratings/78')
    reqlist.append(y3)
    z = requests.delete(url+'rating')
    reqlist.append(z)
    for i in reqlist:
